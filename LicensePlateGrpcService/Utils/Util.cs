...
namespace LicensePlateGrpcService.Utils
{
    public class Util
    {
        static readonly (char from, char to)[] OcrFixMap =
    {
        // Chữ bị OCR nhận nhầm thành số
        ('O','0'), ('I','1'), ('J','3'), ('A','4'),
        ('G','6'), ('B','8'), ('S','5'), ('U','0')
    };

        public static string FormatLicensePlate(string input)
        {
            if (string.IsNullOrWhiteSpace(input))
                return string.Empty;

            // 1️⃣ Bỏ hết ký tự đặc biệt, chỉ giữ lại chữ + số
            string raw = input.ToUpperInvariant();
            raw = Regex.Replace(raw, @"[^A-Z0-9]", "");

            if (raw.Length < 5) return raw;

            // 2️⃣ Sửa lỗi OCR: thay chữ dễ nhầm thành số
            raw = new string(raw.Select(c =>
            {
                var map = OcrFixMap.FirstOrDefault(t => t.from == c);
                return map != default ? map.to : c;
            }).ToArray());

            // 3️⃣ Ghép lại thành format biển số VN: 2 số + 1 chữ + 4–5 số
            var match = Regex.Match(raw, @"^(\d{2})([A-Z])(\d{4,5})$");
            if (match.Success)
                return $"{match.Groups[1].Value}{match.Groups[2].Value}{match.Groups[3].Value}";

            // fallback: trả về chuỗi đã làm sạch
            return raw;
        }
        public static string FormatLicensePlate_BAK(string license)
        {
			var res = "";
			try
			{
